...
def self.uniffi_bytes(v)
  ...
  def self.alloc(size)
    return Math.rust_call(:ffi_math_rustbuffer_alloc, size)
  end

  def self.reserve(rbuf, additional)
    return Math.rust_call(:ffi_math_rustbuffer_reserve, rbuf, additional)
  end

  def free
    Math.rust_call(:ffi_math_rustbuffer_free, self)
  end
...
class RustBufferBuilder
  ...
  def write_U32(v)
    v = Math::uniffi_in_range(v, "u32", 0, 2**32)
    pack_into(4, 'L>', v)
  end

  def write_String(v)
    v = Math::uniffi_utf8(v)
...
module UniFFILib
  ...
  attach_function :uniffi_math_fn_func_add,
    [:uint32, :uint32, RustCallStatus.by_ref],
    :uint32
  attach_function :ffi_math_rustbuffer_alloc,
    [:uint64, RustCallStatus.by_ref],
    RustBuffer.by_value
  attach_function :ffi_math_rustbuffer_from_bytes,
    [ForeignBytes, RustCallStatus.by_ref],
    RustBuffer.by_value
  attach_function :ffi_math_rustbuffer_free,
    [RustBuffer.by_value, RustCallStatus.by_ref],
    :void
  attach_function :ffi_math_rustbuffer_reserve,
    [RustBuffer.by_value, :uint64, RustCallStatus.by_ref],
    RustBuffer.by_value
  attach_function :uniffi_math_checksum_func_add,
    [RustCallStatus.by_ref],
    :uint16
  attach_function :ffi_math_uniffi_contract_version,
    [RustCallStatus.by_ref],
    :uint32
...
def self.add(a, b)
    a = Math::uniffi_in_range(a, "u32", 0, 2**32)
    
    
    b = Math::uniffi_in_range(b, "u32", 0, 2**32)
    
    
  result = Math.rust_call(:uniffi_math_fn_func_add,a,b)
  return result.to_i
end
